...
def photometry(starx, stary, magstar, astrdx, astrdy, radiusCastrd, radiusCstar, radiusastrd, radius, radius2, radius3, buf):
    centroidxstar, centroidystar = calculateCentroid(starx, stary, radiusCstar)
...
##    print deltaxastrd, deltayastrd
    ...
    mask = (x-deltaxstar)**2 +(y+0.5*deltaystar)**2 >= radius**2
    starbox[mask] = 0
    ##print starbox
    ...
    mask2 = (x2-deltaxstar)**2 +(y2+0.5*deltaystar)**2 <= (radius2)**2
    blankbox[mask2] = 0
    mask3 = (x2-deltaxstar)**2 +(y2+0.5*deltaystar)**2 >= (radius3)**2
    blankbox[mask3]=0
    ##print blankbox
    ...
    mask = (x3-deltaxastrd)**2 +(y3+0.5*deltayastrd)**2 >= radiusastrd**2
    astrdbox[mask] = 0
    ##print astrdbox
...
for i in range(1,4):
    for j in range(1,4):
        for k in range(3,7):
            for l in range(3,7):
                magsList.append(photometry(569, 519, 15.49, 463, 468, i, j, k, l, 8, 11, 3))
possibleValue = min(magsList, key=lambda x:abs(x-17.10))
print possibleValue
