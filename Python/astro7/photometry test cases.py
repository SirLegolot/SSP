...
myim = fits.getdata("ApPhotTest.fit")
...
centroidxstar1, centroidystar1 = calculateCentroid(starx1, stary1, radiusCstar)
centroidxstar2, centroidystar2 = calculateCentroid(starx2, stary2, radiusCstar)
centroidxstar3, centroidystar3 = calculateCentroid(starx3, stary3, radiusCstar)

def photometry(centroidxstar, centroidystar, radius, radius2, radius3, buf, starx, stary):
    
    ##centroidxastrd, centroidyastrd = calculateCentroid(astrdx, astrdy, radiusCastrd)
    print "Centroid"
    ...
    print "instmag" # gets the value of constant
    instmag = -2.5*math.log10(signal)
    print instmag

    print "sky brightness"
    skybright = -2.5*math.log10(averageblank)/1.266**2
    print skybright, "mag/arcsec^2"


print "test case 1"
photometry(centroidxstar1, centroidystar1, radius, radius2, radius3, buf, starx1, stary1)
print ""
print ""
print "test case 2"
photometry(centroidxstar2, centroidystar2, radius, radius2, radius3, buf, starx2, stary2)
print ""
print ""
print "test case 3"
photometry(centroidxstar3, centroidystar3, radius, radius2, radius3, buf, starx3, stary3)

    ##print "asteroid box"
    ##astrdbox = myimastrd[astrdy-radiusastrd-buf:astrdy+radiusastrd+buf+1, astrdx-radiusastrd-buf:astrdx+radiusastrd+buf+1] #extracts a 14 by 14 grid around the given asteroid coordinates
    ##y3,x3 =np.ogrid[-radiusastrd-buf:radiusastrd+buf+1,-radiusastrd-buf:radiusastrd+buf+1]
    ##mask = (x3+deltaxastrd)**2 +(y3+deltayastrd)**2 >= radiusastrd**2
    ##astrdbox[mask] = 0
    ##print astrdbox
    ##
    ...
    ##
    ##print "Nap astrd"
    ##Napastrd = np.count_nonzero(astrdbox)
    ##
    ##print "signal asteroid" # calculates the signal of asteroid
    ##signalastrd = totalastrd - (averageblank*(Napastrd))
    ##print signalastrd
    ##
